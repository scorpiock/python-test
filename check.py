...
def check_cert_exp(domain_name):
	...
	t = str(x509.get_notAfter()[:8]).split("'")[1]
...
def print_f(data):
	if data["status"] == "not ok":
		print(f"{data['domain_name']}, Not up")
	elif data["status"] == "ok" and "days" in data:
		print(f"{data['domain_name']},OK,{data['time']} sec,{data['days']}")
	else:
		print(f"{data['domain_name']},OK,{data['time']} sec")
